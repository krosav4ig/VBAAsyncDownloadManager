...
' Назначение: Точка входа приложения. Инициализация менеджера, логгера и запуск цикла.
' ============================================================================
#If VBA7 Then
...
Sub StartBatchDownload()
    ...
    ' maxConcurrent:=1 для теста одной проблемной ссылки
    ' Таймауты настроены с запасом, буфер 64Кб, проверка каждые 5 сек
    Set g_manager = New CAsyncDownloadManager
    g_manager.Init maxConcurrent:=2, callback:=g_logger, _
    ...
    ' Чтение задач из таблицы (предполагается наличие ListObject "Ссылки" на Лист1)
    ...
    Do While g_manager.IsBusy
        DoEvents
        Sleep 100 ' Небольшая пауза для снижения нагрузки на CPU
    Loop
...
' Обработчик потери состояния (вызывается при критических сбоях VBA)
